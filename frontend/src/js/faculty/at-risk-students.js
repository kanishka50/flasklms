document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadAtRiskStudents() {
        try {
            ...
            // If API fails, load from all students and filter
            await loadAllStudentsAndFilter();
        } finally {
            showLoading(false);
        }
    }
    
    async function loadAllStudentsAndFilter() {
        try {
            allAtRiskStudents = [];
            
            // Load all students and identify at-risk ones
                ...
                try {
                    ...
                    if (studentsResponse.status === 'success' && studentsResponse.data.students) {
                        ...
                        studentsResponse.data.students.forEach(student => {
                            ...
                            const riskLevel = calculateRiskLevel(attendance.attendance_rate, student.gpa);
                            
                            // Only include medium and high risk students
                            if (riskLevel === 'medium' || riskLevel === 'high') {
                                allAtRiskStudents.push({
                                    ...student,
                                    ...
                                    risk_factors: identifyRiskFactors(attendance.attendance_rate, student.gpa)
                                });
                            }
                        });
                    }
    ...
    function filterStudents() {
        ...
        filteredStudents = allAtRiskStudents.filter(student => {
            ...
            // Search filter
            if (searchValue) {
                const searchText = `${student.first_name || student.student_name || ''} ${student.last_name || ''} ${student.student_id}`.toLowerCase();
        ...
        card.innerHTML = `
            <div class="p-6">
                <div class="flex justify-between items-start mb-4">
                    <div class="flex items-center">
                        <div class="w-12 h-12 bg-gray-300 rounded-full flex items-center justify-center text-gray-600 font-medium text-lg">
                            ${(student.first_name || student.student_name || 'U')[0]}${(student.last_name || 'U')[0]}
                        </div>
                        <div class="ml-4">
                            <h3 class="text-lg font-semibold text-gray-900">
                                ${student.first_name || student.student_name || 'Unknown'} ${student.last_name || ''}
                            </h3>
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <button onclick="recordIntervention('${student.student_id}', '${student.first_name || student.student_name} ${student.last_name || ''}')" 
    ...
    function exportToCSV() {
        ...
        filteredStudents.forEach(student => {
            ...
            csv += `"${student.student_id}","${(student.first_name || student.student_name) || 'Unknown'} ${student.last_name || ''}","${student.course_code}","${student.risk_level}","${(student.attendance_rate || 0).toFixed(1)}%","${student.gpa || 'N/A'}","${student.predicted_grade}","${riskFactors}"\n`;
        });
